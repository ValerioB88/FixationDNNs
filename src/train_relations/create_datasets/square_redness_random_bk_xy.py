...
import pathlib
import shutil
folder = './data/simple_relations_redness_random_bk_xy/'
...
for i in range(8000):
    ...
    random_xy_target = np.array([np.random.randint(0, img_size[0]//2-size_target), np.random.randint(0, img_size[0]-size_target)])
    ...
    random_xy_relation = np.array([np.random.randint(img_size[0]//2, img_size[0] - size_relation), np.random.randint(0, img_size[0]-size_relation)])
